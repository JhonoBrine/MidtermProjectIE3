...
st.write(
    "You will be able to create your own table, and see it live immediately with the three data visualizations."
)
...
new_column_name = st.sidebar.text_input(
    "Enter new column name (leave blank to skip):", ""
)
if st.sidebar.button("Add Column") and new_column_name:
    if new_column_name not in st.session_state.data.columns:
        st.session_state.data[new_column_name] = [""] * len(
...
if st.sidebar.button("Add Row"):
    new_row_df = pd.DataFrame(
        {col: [""] for col in st.session_state.data.columns}, index=[0]
...
if not st.session_state.data.empty:
    selected_row = st.sidebar.selectbox(
        "Select row to delete:", st.session_state.data.index, key="delete_row_select"
    )
...
if (
    len(st.session_state.data.columns) > 1
):  # Ensure there's more than one column to delete
    columns_to_delete = st.sidebar.selectbox(
        "Select a column to delete:", st.session_state.data.columns
    )
...
if st.sidebar.button("Export to CSV"):
    ...
    st.sidebar.download_button(
        label="Download CSV", data=csv, file_name="current_data.csv", mime="text/csv"
    )
    ...
    try:
        if not data.empty and data.select_dtypes(include=["float", "int"]).shape[1] > 0:
            ...
            fig = ff.create_distplot(
                hist_data, group_labels, bin_size=[0.5] * len(group_labels)
            )
    ...
    try:
        if len(data.select_dtypes(include=["float", "int"]).columns) > 1:
            ...
            fig_heatmap = px.imshow(
                corr, color_continuous_scale="Viridis", aspect="auto", title="Heatmap"
            )
